...
author: "William Imoh"
meta_description: "Ship larger parcels using Porterbuddy's Quite Big Box product."
---

Parcels shipped with Porterbuddy currently fall within the weight limit of 21kg and dimension limit of 60cm x 60cm x 120cm.
This threshold excludes larger parcels in orders from being delivered by Porterbuddy. To solve this, we introduced a new product called "Quite Big Box" (QBB) with an increased capacity to handle larger parcels.

With QBB, we upgraded the parcel requirements to:

- Dimensions: 140cm x 110cm x 7cm
- Weight: 21kg per parcel
- Maximum volume: 450 dm3
- One dimension cannot be more than 70cm

<ContentCallout title={"Maximum order weight 💡"}>While each parcel in an order is limited to 21kg, the total weight of all items in the order must not exceed 25kg.</ContentCallout> 

QBB orders are also referred to as "large".
...
This request creates a QBB order in Porterbuddy's systems.
...
## Activating QBB
To activate Quite Big Box, contact your key account manager, requesting to enable the product on your account.

## Current plugin support
The QBB product is currently supported in the following Porterbuddy plugins:
