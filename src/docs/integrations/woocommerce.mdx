...
2. From the WordPress admin, navigate to _Plugins_ and _activate_ "Porterbuddy for WooCommerce".
...
2. Navigate to the _Plugins_ menu, _Add New,_ then upload the zip file containing the plugin
...
3. Once the installation is complete, navigate to _Plugins_ and click on _activate_ "PorterBuddy for WooCommerce".
...
In this configuration, enter the API keys and token of your Porterbuddy account, both for test and production. Use the _Environment_ setting to toggle between the test and production environments.
...
Find more information about what text can be customized, their keys, and default values [in the documentation](/product-card-widget/).
...
3. Enable “_Hide shipping costs until an address is entered_” in WooCommerce → Settings → Shipping → Shipping options. (Strongly recommended)
...
4. Ensure the Wordpress Admin has the correct Timezone and Site Name: 1. Timezone set up is at in Wordpress → Admin → General → Timezone = Oslo (GMT+2). 2. Ensure the Site Name is correctly set up in Wordpress → Admin → General → Site Name. Porterbuddy uses this name on the origin address of the shipping label.
...
[Here is the public URL](/usm/logos/) to the logo image of a shipping option. Maximum displayable logo size is 70x37 px. The logo url can be specified as relative to the embedded host, or absolute.
