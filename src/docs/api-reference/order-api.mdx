...
| constraints              | No       | [ParcelConstraints](#parcelconstraints) | none    | Constraints for parcel delivery. <br/> \* Some api calls like `/update-shipment-information` will override the existing parcel data, including constraints. Make sure not to change the constraints after the labels for the order are printed. |
